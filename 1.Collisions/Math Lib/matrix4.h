...
struct matrix4
{
   ...
   template <typename VT>
	...
	// Calculates a rotation matrix about the appropriate axis by radians
	// and multiplies it with "this" matrix in the order of (rotation * "this")
	// This is a PRECONCATONATION, and results in an OBJECT SPACE rotation.
	// Object space rotations are useful for rotating an object in place around it's own axes.
	...
	// Calculates a rotation matrix about the appropriate axis by radians
	// and multiplies it with "this" matrix in the order of ("this" * rotation)
	// This is a POSTCONCATONATION, and results in an WORLD SPACE rotation.
	// World space rotations are useful for orbiting an object around a point in space.
	...
	// Calculates a scaling matrix, scaling each axis by the element_type provided for it
	// and multiplies it with "this" matrix in the order of (scale * "this")
	// This is a PRECONCATONATION, and results in an OBJECT SPACE scale.
	reference scale_pre(element_type x, element_type y, element_type z);
   template <typename VT>
	reference scale_pre(const vec3<VT> &v);

	// Calculates a scaling matrix, scaling each axis by the element_type provided for it
	// and multiplies it with "this" matrix in the order of ("this" * scale)
	// This is a POSTCONCATONATION, and results in an WORLD SPACE scale.
	reference scale_post(element_type x, element_type y, element_type z);
	template <typename VT>
	reference scale_post(const vec3<VT> &v);

	// Calculates a translation matrix with the specified translation vector
	// and multiplies it with "this" matrix in the order of (translation * "this")
	// This is a PRECONCATONATION, and results in an OBJECT SPACE translation.
	// Object space translations require vectors that are *relative* to the object.
	// This means that no matter what the object's rotation may currently be
	// in world space, if we want to translate it down it's Z-Axis (for example)
	// we use the vector (0,0,1) or some scale of it.
	reference translate_pre(element_type x, element_type y, element_type z);
	template <typename VT>
	reference translate_pre(const vec3<VT> &v);

	// Calculates a translation matrix with the specified translation vector
	// and multiplies it with "this" matrix in the order of ("this" * translation)
	// This is a POSTCONCATONATION, and results in an WORLD SPACE translation.
	// World space translations require vectors that are in world space.
	// This means that if you wanted to translate an object down it's Z-Axis (for example)
	// then you should translate by the vector representing it's Z-Axis or some scale of it.
	reference translate_post(element_type x, element_type y, element_type z);
	template <typename VT>
	reference translate_post(const vec3<VT> &v);

	// Transpose the matrix, swapping it's values and causing row major to become column major and vice versa.
